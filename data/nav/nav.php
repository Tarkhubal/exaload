<a href="//exaload.glitch.me/login">
    <div class="login-button" align="center" title="Connexion">
        <img class="login-button-img" width="55" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/1828391-origin.png?v=1662620429286">
    </div>
</a>

<nav class="side-nav">
    <div class="wrapper">
        <div id="nav-button-menu" class="nav-bloc n-1">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/Light%20mode.png?v=1656070041573" style="border-radius: 12px;">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me">
                    <h2>Nos Réseaux Sociaux</h2>
                </a>
                <ul>
                    <a class="nav" target="_blank" href="https://www.youtube.com/channel/UCgA-oZcSCmG8ggg4RU99pLg">
                        <li>YouTube</li>
                    </a>
                    <a class="nav" target="_blank" href="https://discord.com/invite/2FjfpXavSF">
                        <li>Discord</li>
                    </a>
                    <a class="nav" target="_blank" href="https://twitter.com/exa_load">
                        <li>Twitter</li>
                    </a>
                    <a class="nav" target="_blank" href="https://instagram.com/exaload">
                        <li>Instagram</li>
                    </a>
                    <a class="nav" target="_blank" href="https://www.tiktok.com/@exaload">
                        <li>TikTok</li>
                    </a>
                    <a class="nav" target="_blank" href="https://www.reddit.com/r/exaload/">
                        <li>Reddit</li>
                    </a>
                </ul>
            </div>
        </div>
        <div id="nav-button-games" class="nav-bloc n-5">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me/games'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/3326678-origin.png?v=1662620103921">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me/games">
                    <h2>Jeux</h2>
                </a>
                <ul>
                    <a class="nav" href="https://exaload.glitch.me/uc/">
                        <li>[element]</li>
                    </a>
                </ul>
            </div>
        </div>
        <div id="nav-button-movies" class="nav-bloc n-3">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me/movies'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/1179120-origin.png?v=1662620104219">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me/movies">
                    <h2>Films</h2>
                </a>
                <ul>
                    <a class="nav" href="https://exaload.glitch.me/uc/">
                        <li>[element]</li>
                    </a>
                </ul>
            </div>
        </div>

        <div id="nav-button-series" class="nav-bloc n-4">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me/uc/'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/5261884-origin.png?v=1662620104130">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me/uc/">
                    <h2>Séries</h2>
                </a>
                <ul>
                    <a class="nav" href="https://exaload.glitch.me/uc/">
                        <li>[element]</li>
                    </a>
                </ul>
            </div>
        </div>
        <div id="nav-button-musics" class="nav-bloc n-6">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me/musics'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/2402461-origin.png?v=1662620104497">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me/musics">
                    <h2>Musiques</h2>
                </a>
                <ul>
                    <a class="nav" href="https://exaload.glitch.me/musics">
                        <li>[element]</li>
                    </a>
                </ul>
            </div>
        </div>
        <div id="nav-button-apps" class="nav-bloc n-7">
            <img class="nav-img" onclick="window.location.href='https://exaload.glitch.me/uc/'" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/1182819-origin.png?v=1662620103815">
            <div class="sub-nav">
                <a class="nav" href="https://exaload.glitch.me/uc/">
                    <h2>Applications</h2>
                </a>
                <ul>
                    <a class="nav" href="https://exaload.glitch.me/uc/">
                        <li>[element]</li>
                    </a>
                </ul>
            </div>
        </div>
    </div>
</nav>
<a href="//exaload.glitch.me/settings">
    <div class="settings-button" align="center" title="Paramètres">
        <img class="settings-button-img" width="55" src="https://cdn.glitch.global/1f61965e-15d8-4de3-8f2d-15723c6634d9/parametres-cog-origin.png?v=1662620104055">
    </div>
</a>